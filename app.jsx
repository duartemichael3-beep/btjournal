...
const R_VALUE = 300
const STORAGE_KEY = "bt_journal_v4"

const DEFAULT_TRADE = { fecha:"",horaInicio:"09:30",horaFinal:"10:00",atr:"",setup:"M1",contexto:"APERTURA",buySell:"BUY",puntosSlStr:"",rResultado:"",resultado:"SL",breakRangoM30:"NO",direccionDia:"RANGO",m5:"",m15:"",m30:"",screenshot:null,screenshotPreview:null,notas:"" }
...
// Get month label short
function getMoLabel(dateStr){
  if(!dateStr)return""
  const d=new Date(dateStr)
  const mm=d.toLocaleString("es",{month:"short"})
  const yy=String(d.getFullYear()).slice(-2)
  return `${mm} ${yy}`
}

// Get year 2-digit
function getYr2(dateStr){
  if(!dateStr)return""
  return String(new Date(dateStr).getFullYear()).slice(-2)
}

// Calc duration in minutes between two HH:MM strings
...
function calcStats(trades){
  if(!trades.length)return{total:0,wins:0,losses:0,bes:0,winRate:0,totalR:0,totalDollar:0,avgR:0,bestR:0,worstR:-1,profitFactor:0,expectancy:0}
  const rs=trades.map(getRforTrade)
  const wins=rs.filter(r=>r>0),losses=rs.filter(r=>r<0),bes=rs.filter(r=>r===0)
  const totalR=Math.round(rs.reduce((a,b)=>a+b,0)*100)/100
  const gw=wins.reduce((a,r)=>a+r,0),gl=Math.abs(losses.reduce((a,r)=>a+r,0))
  return{total:trades.length,wins:wins.length,losses:losses.length,bes:bes.length,winRate:Math.round((wins.length/trades.length)*100),totalR,totalDollar:Math.round(totalR*R_VALUE),avgR:Math.round((totalR/trades.length)*100)/100,bestR:rs.length?Math.max(...rs):0,worstR:rs.length?Math.min(...rs):0,profitFactor:gl?Math.round((gw/gl)*100)/100:gw>0?Infinity:0,expectancy:trades.length?Math.round((totalR/trades.length)*100)/100:0}
}

function groupBy(trades,keyFn){
  const m={};trades.forEach(t=>{const k=keyFn(t);if(k)(m[k]??=[]).push(t)})
  return Object.entries(m).sort((a,b)=>b[0].localeCompare(a[0])).map(([k,ts])=>({key:k,...calcStats(ts)}))
}

// Generate hours every 1 minute
const HOURS=[]
for(let h=0;h<24;h++)for(let m=0;m<60;m++)HOURS.push(`${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}`)
...
function DonutChart({slices,size=150}){
  ...
  const total=valid.reduce((a,s)=>a+s.value,0),R2=size/2-4,r=R2*0.6,cx=size/2,cy=size/2;let angle=-Math.PI/2
  const arcs=valid.map(s=>{const a1=angle,sw=(s.value/total)*Math.PI*2;angle+=sw;const lg=sw>Math.PI?1:0;return{...s,d:`M ${cx+R2*Math.cos(a1)} ${cy+R2*Math.sin(a1)} A ${R2} ${R2} 0 ${lg} 1 ${cx+R2*Math.cos(angle)} ${cy+R2*Math.sin(angle)} L ${cx+r*Math.cos(angle)} ${cy+r*Math.sin(angle)} A ${r} ${r} 0 ${lg} 0 ${cx+r*Math.cos(a1)} ${cy+r*Math.sin(a1)} Z`,pct:Math.round((s.value/total)*100)}})
...
function App(){
  ...
  // Auto-calc duration when hours change
  const setHoraInicio=(v)=>setForm(f=>{const dur=calcDuration(v,f.horaFinal);return{...f,horaInicio:v,duracionTrade:dur!==""?String(dur):""}})
  const setHoraFinal=(v)=>setForm(f=>{const dur=calcDuration(f.horaInicio,v);return{...f,horaFinal:v,duracionTrade:dur!==""?String(dur):""}})

  const filtered=useMemo(()=>{
    let ft=[...trades]
    if(fSetup!=="all")ft=ft.filter(t=>t.setup===fSetup)
    if(fPeriod!=="all"){
      const now=new Date()
      if(fPeriod==="week"){const w=new Date(now-7*864e5);ft=ft.filter(t=>new Date(t.fecha)>=w)}
      else if(fPeriod==="month")ft=ft.filter(t=>{const d=new Date(t.fecha);return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear()})
      else if(fPeriod==="year")ft=ft.filter(t=>new Date(t.fecha).getFullYear()===now.getFullYear())
    }
    return ft.sort((a,b)=>new Date(b.fecha)-new Date(a.fecha))
  },[trades,fPeriod,fSetup])
  ...
  // Weekly = by week-of-month + month label
  const weekly=useMemo(()=>groupBy(trades,t=>{
    if(!t.fecha)return""
    const wk=weekOfMonth(t.fecha)
    const ml=getMoLabel(t.fecha)
    return `S${wk} ${ml}`
  }),[trades])

  const monthly=useMemo(()=>groupBy(trades,t=>getMoLabel(t.fecha)),[trades])

  const yearly=useMemo(()=>groupBy(trades,t=>{
    if(!t.fecha)return""
    return `20${getYr2(t.fecha)}`
  }),[trades])
  ...
  const save=()=>{
    ...
    const trade={...form}
    // Auto-set semana
    trade.semana=String(weekOfMonth(form.fecha))
    // Auto-set duration
    trade.duracionTrade=String(calcDuration(form.horaInicio,form.horaFinal))
    if(trade.resultado==="SL")trade.rResultado="-1"
    if(trade.resultado==="BE")trade.rResultado="0"
    if(editId){setTrades(ts=>ts.map(t=>t.id===editId?{...trade,id:editId}:t));setEditId(null)}
    else setTrades(ts=>[...ts,{...trade,id:uid()}])
    setForm({...DEFAULT_TRADE});setTab("trades")
  }
  ...
  const exportCSV=()=>{
    const h=["fecha","semana","horaInicio","horaFinal","duracionTrade","atr","setup","contexto","buySell","puntosSlStr","rResultado","resultado","breakRangoM30","direccionDia","m5","m15","m30","notas"]
  ...
  const autoDur=calcDuration(form.horaInicio,form.horaFinal)
  const autoWeek=weekOfMonth(form.fecha)

  const nav=[{id:"dashboard",label:"Dashboard",icon:"◈"},{id:"trades",label:"Trades",icon:"☰"},{id:"addTrade",label:editId?"Editar":"Nuevo",icon:"+"},{id:"estadisticas",label:"Stats",icon:"▥"},{id:"setups",label:"Setups",icon:"◆"}]

  // Stats table renderer
  const StatsTable=({title,data,cols,row,chart})=>(
    <div className="card">
      ...
      <div style={{display:"grid",gridTemplateColumns:chart?"minmax(0,1.3fr) minmax(0,1fr)":"1fr",gap:16}}>
        <div style={{overflowX:"auto"}}>
          <table className="tbl"><thead><tr>{cols.map(h=><th key={h}>{h}</th>)}</tr></thead>
            <tbody>{data.map((d,i)=><tr key={i}>{row(d).map((cell,j)=>{
              if(Array.isArray(cell))return<td key={j} className={`mono ${cell[1]} ${cell[2]?"bold":""}`}>{cell[0]}</td>
              return<td key={j} className="mono">{cell}</td>
            })}</tr>)}</tbody>
          </table>
          {!data.length&&<div className="empty-msg">Sin datos</div>}
        </div>
        {chart&&<BarChart data={chart.slice(0,12).reverse().map(w=>w.totalR)} labels={chart.slice(0,12).reverse().map(w=>w.key)} unit="R"/>}
      </div>
    </div>
  )
...
.auto-field{background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:10px 12px;font-family:var(--mono);font-size:13px;color:var(--accent)}
@keyframes fadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}.card,.metric-card{animation:fadeIn .3s ease both}`}</style>
    ...
    <div className="shell">
      ...
      <div className={`sidebar ${sidebar?"open":"closed"}`}>
        ...
        <nav className="sb-nav">{nav.map(n=><button key={n.id} className={`sb-btn ${tab===n.id?"active":""}`} onClick={()=>goTab(n.id)}><span style={{fontFamily:"var(--mono)",fontSize:16,width:20,textAlign:"center"}}>{n.icon}</span><span>{n.label}</span></button>)}</nav>
      ...
      <div className={`main ${!sidebar||window.innerWidth<=900?"full":""}`}>
        ...
        {tab==="dashboard"&&<>
          ...
          <div className="card"><div className="section-title">P&L diario (R)</div><BarChart data={daily.slice(0,20).reverse().map(d=>d.totalR)} labels={daily.slice(0,20).reverse().map(d=>fmtDate(d.key))} unit="R"/></div>
          <div className="card" style={{marginTop:16}}>
            ...
            <div style={{overflowX:"auto"}}><table className="tbl"><thead><tr><th>Fecha</th><th>Sem</th><th>Setup</th><th>B/S</th><th>SL pts</th><th>R</th><th>P&L</th><th>Resultado</th><th>Dir</th><th>Dur</th></tr></thead><tbody>
              {filtered.slice(0,10).map(t=>{const r=getRforTrade(t);return<tr key={t.id} style={{cursor:"pointer"}} onClick={()=>edit(t)}><td className="mono">{fmtDate(t.fecha)}</td><td className="mono">S{weekOfMonth(t.fecha)}</td><td><SetupTag setup={t.setup}/></td><td><BuySellTag bs={t.buySell}/></td><td className="mono">{t.puntosSlStr}</td><td className="mono bold" style={{color:r>0?"var(--green)":r<0?"var(--red)":"var(--yellow)"}}>{fmtR(r)}</td><td className="mono bold" style={{color:r>=0?"var(--green)":"var(--red)"}}>{fmt$(r*R_VALUE)}</td><td><ResTag res={t.resultado}/></td><td><DirTag dir={t.direccionDia}/></td><td className="mono">{t.duracionTrade?t.duracionTrade+"m":""}</td></tr>})}
            </tbody></table></div>
        ...
        {tab==="addTrade"&&<>
          ...
          <div className="card">
            ...
            <div className="form-grid">
              {F("Fecha","fecha","date")}
              <div className="field"><label>Semana (auto)</label><div className="auto-field">S{autoWeek||"—"}</div></div>
              <TimePicker label="Hora inicio" value={form.horaInicio} onChange={setHoraInicio}/>
              <TimePicker label="Hora final" value={form.horaFinal} onChange={setHoraFinal}/>
              <div className="field"><label>Duración (auto)</label><div className="auto-field">{autoDur!==""?autoDur+"min":"—"}</div></div>
              {F("ATR","atr","number")}
            </div>
          </div>
          <div className="card"><div className="section-title">Trade</div><div className="form-grid">{F("Setup","setup",null,SETUPS)}{F("Contexto","contexto",null,CONTEXTOS)}{F("Buy / Sell","buySell",null,["BUY","SELL"])}{F("Puntos SL","puntosSlStr","number")}</div></div>
          <div className="card">
            <div className="section-title">Resultado</div>
            <div className="form-grid">
              {F("Resultado","resultado",null,RESULTADOS)}
              {isTP&&F("R ganados","rResultado","number")}
              {F("Break rango M30","breakRangoM30",null,["NO","SI"])}
              {F("Dirección día","direccionDia",null,DIRECCIONES)}
            </div>
            {form.resultado==="SL"&&<p style={{marginTop:12,fontSize:12,color:"var(--red)",fontFamily:"var(--mono)"}}>SL = -1R = -{fmt$(R_VALUE)}</p>}
            {form.resultado==="BE"&&<p style={{marginTop:12,fontSize:12,color:"var(--yellow)",fontFamily:"var(--mono)"}}>BE = 0R = $0</p>}
            {isTP&&pn(form.rResultado)>0&&<p style={{marginTop:12,fontSize:12,color:"var(--green)",fontFamily:"var(--mono)"}}>+{form.rResultado}R = +{fmt$(pn(form.rResultado)*R_VALUE)}</p>}
          </div>
        ...
        {tab==="estadisticas"&&<>
          ...
          <StatsTable title="Por día" data={daily.slice(0,30)} cols={["Fecha","Trades","W","L","Win%","R","P&L","PF"]}
            row={d=>[fmtDate(d.key),d.total,[d.wins,"g"],[d.losses,"r"],[`${d.winRate}%`,d.winRate>=50?"g":"r"],[`${d.totalR>0?"+":""}${d.totalR}R`,d.totalR>=0?"g":"r",true],[fmt$(d.totalDollar),d.totalDollar>=0?"g":"r"],d.profitFactor===Infinity?"∞":d.profitFactor]}/>
          <StatsTable title="Por semana (del mes)" data={weekly} cols={["Semana","Trades","Win%","R","P&L","PF"]}
            row={w=>[w.key,w.total,[`${w.winRate}%`,w.winRate>=50?"g":"r"],[`${w.totalR>0?"+":""}${w.totalR}R`,w.totalR>=0?"g":"r",true],[fmt$(w.totalDollar),w.totalDollar>=0?"g":"r"],w.profitFactor===Infinity?"∞":w.profitFactor]} chart={weekly}/>
          <StatsTable title="Por mes" data={monthly} cols={["Mes","Trades","Win%","R","P&L","PF"]}
            row={m=>[m.key,m.total,[`${m.winRate}%`,m.winRate>=50?"g":"r"],[`${m.totalR>0?"+":""}${m.totalR}R`,m.totalR>=0?"g":"r",true],[fmt$(m.totalDollar),m.totalDollar>=0?"g":"r"],m.profitFactor===Infinity?"∞":m.profitFactor]} chart={monthly}/>
          <StatsTable title="Por año" data={yearly} cols={["Año","Trades","Win%","R","P&L","PF"]}
            row={y=>[y.key,y.total,[`${y.winRate}%`,y.winRate>=50?"g":"r"],[`${y.totalR>0?"+":""}${y.totalR}R`,y.totalR>=0?"g":"r",true],[fmt$(y.totalDollar),y.totalDollar>=0?"g":"r"],y.profitFactor===Infinity?"∞":y.profitFactor]} chart={yearly}/>
        </>}
